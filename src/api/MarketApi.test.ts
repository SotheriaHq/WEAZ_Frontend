...
describe('toMarketItem', () => {
  it('uses primaryMedia preview and aspect metadata for masonry cards', () => {
    const item = toMarketItem({
      ...
      primaryMedia: {
        fileId: 'file-1',
        displayUrl: 'https://cdn.threadly.test/file-1-detail.webp',
        previewUrl: 'https://cdn.threadly.test/file-1-card.webp',
        type: 'POST_IMAGE',
        width: 900,
        height: 1200,
        aspectRatio: 0.75,
      },
      mediaUrl: 'https://cdn.threadly.test/file-1-original.jpg',
    });

    expect(item.media.fileId).toBe('file-1');
    expect(item.media.url).toBe('https://cdn.threadly.test/file-1-detail.webp');
    expect(item.media.previewUrl).toBe('https://cdn.threadly.test/file-1-card.webp');
