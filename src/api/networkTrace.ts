...
type NetworkTraceSummary = {
  ...
  visibilityCounts: Array<{ visibilityState: string; count: number }>;
...
let lastVisibilityTransition: { state: string | null; at: number } = {
  state: typeof document !== 'undefined' ? document.visibilityState : null,
  at: Date.now(),
};
const entries: NetworkTraceEntry[] = [];

if (isTraceEnabled && typeof document !== 'undefined') {
...
function detectCacheBust(queryParamNames: string[], headers: TraceHeaders | undefined) {
  ...
  for (const name of queryParamNames) {
    if (CACHE_BUST_QUERY_NAMES.has(name.toLowerCase())) {
...
function getRouteLabel() {
  if (currentRouteLabel) return currentRouteLabel;
...
function inferTrigger() {
  ...
  const now = Date.now();
  if (lastVisibilityTransition.state === 'visible' && now - lastVisibilityTransition.at <= 5000) {
    return 'foreground';
  }
  return 'unknown';
}
...
export function finishNetworkTrace(
  ...
  pushEntry({
    ...
    visibilityState: trace.visibilityState,
...
export function setNetworkTraceRoute(label: string | null) {
  currentRouteLabel = label && label.trim().length > 0 ? label.trim() : null;
}
...
export function getNetworkTraceSummary(): NetworkTraceSummary {
  ...
  return {
    ...
    visibilityCounts: countBy(captured, 'visibilityState', 'unknown').map(({ label, count }) => ({ visibilityState: label, count })),
