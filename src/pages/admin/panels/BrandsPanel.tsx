 ...
 *
 * Brand accounts are users of `type=BRAND`; each row opens the shared
 * AccountManageModal (owner lifecycle + storefront controls). Extracted from
 * the old AdminBrandsPage (the verification-queue block moved to InReviewPanel).
 */
...
const BrandsPanel: React.FC = () => {
  ...
  const fetchPage = useCallback(
    async (cursor?: string, limit?: number) => {
      ...
      const data = unwrapApiResponse<{ items?: AdminBrand[]; nextCursor?: string } | AdminBrand[]>(res.data as any);
  ...
  const filteredBrands = useMemo(() => {
    ...
    if (statusFilter !== 'ALL') next = next.filter((brand) => normalizeStatus(brand.owner?.status) === statusFilter);
    next = [...next].sort((a, b) => {
  ...
  const getOwnerName = (brand: AdminBrand) => `${brand.owner?.firstName ?? ''} ${brand.owner?.lastName ?? ''}`.trim() || 'Unknown owner';
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="overflow-hidden rounded-2xl border border-gray-200/80 bg-white/90 shadow-md shadow-gray-200/40 dark:border-white/10 dark:bg-white/[0.03] dark:shadow-none">
        <div className="border-b border-gray-150/70 bg-gray-50/50 p-4 dark:border-white/5 dark:bg-white/[0.02]">
          <div className="grid grid-cols-2 gap-3 md:grid-cols-2 lg:grid-cols-5">
            <input
              ...
              className="col-span-2 w-full rounded-xl border border-gray-200 bg-white px-3 py-2 text-sm text-gray-900 outline-none focus:border-indigo-400 md:col-span-1 dark:border-white/10 dark:bg-black/20 dark:text-white"
            />
            <FilterDropdown value={storeFilter} onChange={(value) => setStoreFilter(value as StoreFilter)} options={STORE_OPTIONS} className="w-full rounded-xl border border-gray-200 bg-white px-3 py-2 text-sm text-gray-900 dark:border-white/10 dark:bg-black/20 dark:text-white" />
            <FilterDropdown value={statusFilter} onChange={(value) => setStatusFilter(value as StatusFilter)} options={STATUS_OPTIONS} className="w-full rounded-xl border border-gray-200 bg-white px-3 py-2 text-sm text-gray-900 dark:border-white/10 dark:bg-black/20 dark:text-white" />
            <FilterDropdown value={sortBy} onChange={(value) => setSortBy(value as SortBy)} options={SORT_OPTIONS} className="w-full rounded-xl border border-gray-200 bg-white px-3 py-2 text-sm text-gray-900 dark:border-white/10 dark:bg-black/20 dark:text-white" />
            <div className="flex gap-2">
              <button type="button" onClick={resetFilters} className="flex-1 rounded-xl border border-gray-300 bg-white px-3 py-2 text-xs font-semibold text-gray-700 hover:bg-gray-50 dark:border-white/20 dark:bg-white/5 dark:text-gray-200 dark:hover:bg-white/10">Reset</button>
              <div className="inline-flex items-center rounded-xl border border-gray-200/80 bg-white p-1 text-xs font-semibold dark:border-white/10 dark:bg-white/5">
                <button type="button" onClick={() => setViewMode('table')} className={`rounded-lg px-2 py-1 transition ${viewMode === 'table' ? 'bg-indigo-600 text-white' : 'text-gray-600 dark:text-gray-300'}`}>📋</button>
                <button type="button" onClick={() => setViewMode('cards')} className={`rounded-lg px-2 py-1 transition ${viewMode === 'cards' ? 'bg-indigo-600 text-white' : 'text-gray-600 dark:text-gray-300'}`}>⊞</button>
              </div>
            </div>
          </div>
        </div>
        ...
        {error && (
          <div className="m-4 rounded-xl border border-rose-200 bg-rose-50 px-3 py-2 text-sm text-rose-700 dark:border-rose-500/40 dark:bg-rose-500/10 dark:text-rose-300">{error}</div>
        )}
                ...
                {filteredBrands.map((brand) => {
                  ...
                  return (
                    <tr key={brand.id} className="border-b border-gray-100/90 transition-colors even:bg-gray-50/40 hover:bg-indigo-50/50 dark:border-white/5 dark:even:bg-white/[0.02] dark:hover:bg-white/5">
                        ...
                        <div className="flex items-center gap-3">
                          <div className="h-11 w-11 shrink-0 overflow-hidden rounded-xl border border-gray-200/70 bg-gray-100 dark:border-white/10 dark:bg-white/10">
                            {visual ? (
                              ...
                              <div className="flex h-full w-full items-center justify-center text-sm font-bold text-gray-500 dark:text-gray-300">{String(brand.name ?? 'BR').slice(0, 2).toUpperCase()}</div>
                            )}
                          </div>
                          <div><p className="font-semibold text-gray-900 dark:text-white">{brand.name || 'Unnamed brand'}</p></div>
                        </div>
                      </td>
                      <td className="px-3 py-3">
                        {brand.owner?.id ? (
                          <div className="flex items-center gap-1.5 font-mono text-xs">
                            <Link to={`/u/${brand.owner.username}`} className="font-semibold text-indigo-600 hover:underline dark:text-indigo-400">{brandUid}</Link>
                            <button type="button" onClick={(e) => { e.preventDefault(); e.stopPropagation(); void navigator.clipboard.writeText(brandUid); toast.success('UID copied to clipboard!'); }} title="Copy UID" className="text-gray-400 transition-colors hover:text-gray-600 dark:hover:text-gray-200">📋</button>
                          </div>
                        ) : '—'}
                      </td>
                      ...
                      <td className="px-3 py-3">
                        <span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-semibold ${ownerStatus === 'ACTIVE' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-200' : ownerStatus === 'SUSPENDED' ? 'bg-amber-100 text-amber-700 dark:bg-amber-500/20 dark:text-amber-200' : ownerStatus === 'DEACTIVATED' ? 'bg-rose-100 text-rose-700 dark:bg-rose-500/20 dark:text-rose-200' : 'bg-gray-100 text-gray-600 dark:bg-white/10 dark:text-gray-300'}`}>
