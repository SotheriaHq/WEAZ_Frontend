...
const signupSchema = z.object({
  ...
  password: z
    ...
    .min(PASSWORD_POLICY_MIN_LENGTH, { message: getPasswordPolicyErrorMessage('Password') })
...
const SignUpPage = () => {
  ...
  const handleGoogleSignup = async () => {
    ...
    if (!selectedType) {
      setError('userType', { type: 'manual', message: 'Please select an account type' });
      toast.error('Choose an account type before continuing with Google.');
      return;
    }

    if (selectedType === 'BRAND' && brandFullName.length < 2) {
      setError('brandFullName', { type: 'manual', message: 'Brand name is required' });
      toast.error('Enter your brand name before continuing with Google.');
      return;
    }

    if (!agreedToTerms) {
      setError('agreeTerms', { type: 'manual', message: 'You must agree to the terms' });
      toast.error('Agree to the Terms and Privacy Policy before continuing with Google.');
      return;
    }
    ...
    try {
      ...
      if (isAxiosError(error)) {
        ...
        toast.error(typeof message === 'string' ? message : 'Google signup could not be completed.');
      }
    } finally {
          ...
          <div className="w-full max-w-md mt-16 lg:mt-0">
            ...
            <div className="auth-glass-panel rounded-2xl p-8 sm:p-10 w-full">
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                />
                <button type="button" disabled className="auth-social-btn flex items-center justify-center gap-3 py-3 rounded-xl text-sm font-medium group cursor-not-allowed opacity-60">
