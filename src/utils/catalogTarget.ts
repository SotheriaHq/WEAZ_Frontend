...
import { CATALOG_ENTITY_TYPES, isCatalogEntityType } from '@/constants/catalogDomain';
...
export function normalizeCatalogTarget(input: CatalogTargetInput): NormalizedCatalogTarget | null {
  ...
  if (normalizeId(input.designId)) {
    return normalizeCatalogTarget({ ...input, targetType: 'DESIGN' });
  }
  if (normalizeId(input.productId)) {
    return normalizeCatalogTarget({ ...input, targetType: 'PRODUCT' });
  }
  if (normalizeId(input.collectionId)) {
    return normalizeCatalogTarget({ ...input, targetType: 'COLLECTION' });
  }
