...
# Browsers keep hashed assets for a year; Cloudflare's EDGE revalidates every
# 5 minutes. INCIDENT 2026-07-10: during a deploy the SPA fallback (index.html)
# was cached AT /assets/*.js URLs under the immutable header, breaking boot for
# every user until a manual purge. The short edge TTL makes that self-heal.
/assets/*
